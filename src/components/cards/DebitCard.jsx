    ...
    <div>
      <div className="flex justify-end mb-2">
        <button
          ...
          className="flex items-center gap-1.5 text-sm font-semibold text-aspire-green hover:opacity-80 transition-opacity"
        >
      ...
      <div
        className={`relative rounded-2xl p-6 overflow-hidden select-none transition-all duration-300
      ...
      >
        ...
        <div className="absolute -bottom-8 -right-2 w-28 h-28 rounded-full bg-white/10 pointer-events-none" />
        ...
        <p className="text-white text-lg font-bold mb-5 relative z-10">
          {card.cardholderName}
        </p>

        <div className="flex items-center gap-3 mb-3 relative z-10 font-mono">
          {displayNumber.split(' ').map((group, i) => (
            <span key={i} className="text-white text-sm tracking-widest">{group}</span>
          ))}
        </div>
        ...
        <div className="absolute bottom-5 right-6 z-10">
          <span className="text-white text-2xl font-black italic">VISA</span>
        </div>
      </div>
    </div>
  )
